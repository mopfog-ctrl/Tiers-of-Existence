 ...
 */
class TurnDriver(
    private val decisions: TurnDecisionProvider,
    private val rollForPhase: (Phase) -> Int = { Dice.rollForPhase(it) },
) {

    /**
     * Drives exactly one player's turn to completion: rolls, asks [decisions] which eligible
     * token to move, moves it, resolves whatever that landing produces (including asking
     * [decisions] about any optional offer), then ends the turn — chaining another turn for the
     * same player on a "Go again" square, otherwise advancing to the next eligible player/Phase
    ...
    fun driveOneTurn(state: GameState): Boolean {
        ...
        val grantAnotherTurn = moveAndResolve(state, player, chosen, roll.total)
    ...
    private fun moveAndResolve(state: GameState, player: PlayerColor, id: TokenId, spaces: Int): Boolean =
        when (id.kind) {
            TokenKind.TIER_TOKEN -> {
                ...
                moveResult?.let { resolveTierEffect(state, player, id.tier, it) } ?: false
            }
            TokenKind.MARAUDER -> resolveMarauderEffect(state, player, id.tier, TurnEngine.moveMarauderById(state, id, spaces))
        }
    ...
    private fun resolveTierEffect(state: GameState, player: PlayerColor, tier: TierLevel, result: MoveResult): Boolean {
    ...
    private fun resolveMarauderEffect(state: GameState, player: PlayerColor, tier: TierLevel, result: MoveResult): Boolean {
